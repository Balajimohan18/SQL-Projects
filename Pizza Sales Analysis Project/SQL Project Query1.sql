...
SELECT pizza_category AS Pizza_Category, SUM(quantity) AS Total_Pizza_Sold FROM pizza_sales
GROUP BY Pizza_Category
...
SELECT Top 5 pizza_name as Pizza_Name , CAST(SUM(total_price) AS DECIMAL(10,2)) AS Total_Revenue FROM pizza_sales
GROUP BY pizza_name
ORDER BY Total_Revenue

--Bottom 5 Pizzas by Total Quantity
SELECT Top 5 pizza_name as Pizza_Name , SUM(quantity) AS Total_Quantity FROM pizza_sales
GROUP BY pizza_name
